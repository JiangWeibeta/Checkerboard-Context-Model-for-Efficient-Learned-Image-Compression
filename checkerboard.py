...
class Checkerboard(models.JointAutoregressiveHierarchicalPriors):
    def __init__(self, N=128, M=192, height=256, width=256, batch_size=8, **kwargs):
        ...
        self.zeros = torch.zeros([int(batch_size), M * 2, int(height) // 16, int(width) // 16], requires_grad=False).cuda()
        ...
        # self.register_buffer('mask', torch.zeros([batch_size, self.M, height // 16, width // 16]).clone())
        # self.register_buffer('remask', torch.zeros([batch_size, self.M, height // 16, width // 16]).clone())
        # self.register_buffer('context_mask', torch.zeros([batch_size, M * 2, height // 16, width // 16]).clone())
        # self.register_buffer('zeros', torch.zeros([int(batch_size), M * 2, int(height) // 16, int(width) // 16]).clone())
