...
class Checkerboard(models.JointAutoregressiveHierarchicalPriors):
    ...
    def forward(self, x):
        ...
        masked_context *= self.context_mask

        pseudo_gaussian_params = self.entropy_parameters(
        ...
        pseudo_scales, pseudo_means = pseudo_gaussian_params.chunk(2, 1)
        scales, means = gaussian_params.chunk(2, 1)

        pseudo_scales = pseudo_scales * self.mask
        pseudo_means = pseudo_means * self.mask
        scales_hat = scales * self.remask
        means_hat = means * self.remask

        true_scales = pseudo_scales + scales_hat
        true_means = pseudo_means + means_hat

        _, y_likelihoods = self.gaussian_conditional(y, true_scales, means=true_means)
    ...
    def compress_slice_concatenate(self):
        ...
        y_b = torch.zeros([self.batch_size, self.M * 2, self.height // 32, self.width // 32])
        y_c = torch.zeros([self.batch_size, self.M * 2, self.height // 32, self.width // 32])
        scales_b = torch.zeros([self.batch_size, self.M * 2, self.height // 32, self.width // 32])
        scales_c = torch.zeros([self.batch_size, self.M * 2, self.height // 32, self.width // 32])
        means_b = torch.zeros([self.batch_size, self.M * 2, self.height // 32, self.width // 32])
        means_c = torch.zeros([self.batch_size, self.M * 2, self.height // 32, self.width // 32])
        y_b[:, :, :, :] = y[:, :, 0::2, 1::2]
        y_c[:, :, :, :] = y[:, :, 1::2, 0::2]
        ...
        y_anchor_quantized = torch.zeros([self.batch_size, self.M, self.height // 16, self.width // 16])
        ...
        y_a = torch.zeros([self.batch_size, self.M * 2, self.height // 32, self.width // 32])
        y_d = torch.zeros([self.batch_size, self.M * 2, self.height // 32, self.width // 32])
        scales_a = torch.zeros([self.batch_size, self.M * 2, self.height // 32, self.width // 32])
        scales_d = torch.zeros([self.batch_size, self.M * 2, self.height // 32, self.width // 32])
        means_a = torch.zeros([self.batch_size, self.M * 2, self.height // 32, self.width // 32])
        means_d = torch.zeros([self.batch_size, self.M * 2, self.height // 32, self.width // 32])
        y_a[:, :, :, :] = y[:, :, 0::2, 0::2]
        y_d[:, :, :, :] = y[:, :, 1::2, 1::2]
        ...
        return {"strings": [y_a_strings, y_b_strings, y_c_strings, y_d_strings, z_strings], "shape": z.size()[-2:]}, hyper_info

    def decompress_slice_concatenate(self, strings, shape):
        start_time = time.clock()

        y_quantized = torch.zeros([self.batch_size, self.M, self.height // 16, self.width // 16])
